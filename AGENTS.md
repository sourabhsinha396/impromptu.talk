...
1. **Parity first, and v0 is the record, not the authority.** Every v0 feature ships in v1 before any new one is discussed: nothing is dropped. But v0 had bad decisions, and this rebuild exists to improve the UI, the backend, the tables, everything. When a v0 choice was bad, improve it and log the change in `docs/DECISIONS.md`. "v0 did it" or "the mock had it" is never a reason on its own; say why it is right. v0 is read-only: read its behaviour, rules, copy, algorithms and tests; never copy files, never copy the UI. v0 was FastAPI and Jinja and this is Next.js and Django Ninja, so much is different by design, and that is fine. Before a card builds a surface, a mock of it lands as a standalone HTML page under `docs/mocks/`, the owner approves it, and `docs/UI.md` records what changed and why. `docs/tech/v0-parity.md` is the acceptance list for behaviour.
