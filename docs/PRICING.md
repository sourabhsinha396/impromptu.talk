...
**The streak's length and its freezes are counting rules, not stored state.** Free counts at most five days back; a Pro plan counts as many days as it lasts, a year at most. A Pro streak survives two missed days in any thirty. Two covers a weekend away, which is what people actually lose streaks to; a rolling thirty days rather than per streak, so a long streak keeps being forgiven; small enough that practising every other day still breaks, which a streak that never breaks would not be worth having. The window rolls back from the gap and does not reset on the 1st, because a calendar month gave the same three days away two allowances when they crossed a boundary and one when they did not, and nobody plans an absence around the date. There is no table, no balance and no SKU, because an inventory that can be spent, refunded, granted and drift out of step with the runs it protects is exactly the stored state the streak code exists to avoid. It cannot disagree with the runs table, buying Pro repairs the gaps already behind you, and there is nothing to draw a "two left" control for.

**The share page is not a tier.** `/s/<token>` shows the same eight weeks to everybody, whatever the owner's plan, and a lapsed Pro's link keeps working with the same eight weeks. Pointing it at the free window would silently shrink every card ever sent the next time that window moved; the link is the growth surface, and nothing is sold on it.
