...
General, Relationships, Career and work, Money and business, Tech and AI, Science and climate, Health and mind, Philosophy and ethics, Culture, Deep research.

The ten are the flat unheaded list; **warm-ups sit in a second labelled group beneath them**, which is the shelf **Yours** already built (§8) and its second tenant, so adding one does not reopen the decision behind ten. Shipped as twenty in v0 and cut to ten on 5 September 2026. A flat scroll list stops working somewhere around fifteen, which is exactly why ten needs no shelves: there are no category headers in the picker, no search box and no blurbs (ten names fit on one screen and name themselves). The blurb earns its place on the genre page, where somebody arriving from a search engine has to be told what they landed on.

**Merged, never deleted.** The ten that went took their topics into the ten that stayed: food and travel into Everyday life; productivity into Career; personal finance and startups into Money and business; climate into Science; fitness, mental health and psychology into Health and mind; ethics into Philosophy; history, film and TV, anime and manga into Culture; and, on 9 September 2026, Everyday life itself into General, which had been holding the same ordinary objects and moments under a blurb that promised everything. The bank was 800 in v0 and is over 1500 in v1 (owner's call, 6 September 2026: 200 written for the rebuild, the thin genres first; the picture bank and Deep research since). Forty stays the floor, and the 80 to 120 band is retired: General carries the widest bank because it is the default a stranger lands on without opening the picker, and the count a genre holds is not a number worth balancing. Anime and manga was the one contested call and is the first to split out again if genre pages earn their traffic.
...
**A feature that changes what the round *is* gets a URL of its own; a preference that tunes a round stays a setting.** Home is the tool and keeps the genre chip, the question and one button, so a mode added there is paid for out of the one screen that must not get heavier: a checkbox on the settings sheet, a row in the picker, and a name nobody can search for. A feature page carries its own heading, its own settings and its own bank, server-rendered, and the genre is fixed to the page rather than picked. **It opens on the tool**: the server picks what the round starts on, so the first paint is already the thing you came for rather than a landing screen that spins itself away. A feature page has no idle phase, and the way out of one prompt is the next one. Its bank sits below the fold as plain text, which is what a crawler reads and what earns the URL its place.

The test is what the round becomes. Tongue twisters is a different round, so it is `/tongue-twisters`. Thinking time, talking time, sound and the accent tune a round that is otherwise the same, so they stay settings. Image impromptu is a different round and is still only `/?pictures=1`, which is the shape this rule exists to stop; it takes a URL when its card comes up.

A feature's genre is **a link in the picker, not an option**, with a chevron, because a genre is something you pick and stay here for and a feature is a page you leave for. `/?genre=<a feature>` redirects to it.

### Warm-ups, and the scroller

A **warm-up** is a genre whose topics are passages you read aloud rather than prompts you talk about. One nullable column, `Genre.mode`, holding `speak` (the ten, and everything anybody owns) or `read`. **Tongue twisters** is the first and only one.

- **The scroll is the timer, so there is no clock.** The words move upward at a speed you pick and keeping up with them is the whole task. Falling behind is the failure and you can hear it happen, so nothing measures it: no microphone, no transcript, no model, nothing against the transcription allowance. A ring would have been the wrong instrument, since a ring measures a fixed length you are trying to fill and this is a fixed text you are trying to keep pace with.
- **Speed is the difficulty.** Four speeds in words a minute (120, 150, 180, 220), a segment rather than a slider, remembered in the prefs. The same passage is gentle at 120 and brutal at 220, which is why there is no difficulty column to author.
- **A passage is 100 to 120 words**, which runs 53 to 58 seconds at 120 and about 30 at 220. Easy and hard are a label on the row, in `Topic.style` (the only axis a passage has: nobody chooses how to say words they are reading verbatim). Both are free, always. Twenty-two passages, six easy and sixteen hard, each built around one thing a mouth does badly rather than being long for its own sake. **A word too long for the column takes a size of its own and stays whole**, because a tongue twister has to be taken in as one unit to be attacked; the passage around it is untouched.
- **The middle of the round differs and nothing else does.** Idle asks "Can you keep up?"; the spin lands with no reel, because a strip of one-line prompts cannot honestly stop on a paragraph; the passage screen shows the words, the speed and how long the clip will run; a three-second lead-in lets somebody press record and get ready; then the scroll, full bleed, chrome hidden, masked top and bottom. Done says the passage, the speed and the word count, and says plainly that nothing was recorded so only the reader knows whether they kept up. **Again leads and Spin is the ghost**, the reverse of the round's done screen, because a warm-up's loop is repetition rather than variety.
- **A warm-up does not build the streak.** A 45-second read is not a minute of unscripted speaking, and letting it count would make the streak mean something weaker. No run is written, so there is nothing to count; the fastest speed per passage lives in the browser under `yapholic.warmups`. The last line of the done screen routes into a real round and says why.
- **The passages are fetched when the genre is picked**, not shipped inline with the bank, because home carries the whole bank on every visit and most visitors never open this genre. `pool` never falls back to a sentence for a read genre, the same exception picture mode has, and the Spin button waits until the rows land rather than pressing to nothing.
- **`/tongue-twisters` is the page**, not `/genre/tongue-twisters`: the URL has to survive being typed into a comment reply. Every passage in full as plain text, easy first, one button into the tool, in the sitemap beside the genre pages, and a card on `/features`.
- **Making your own is Pro**, and it is an owned genre with `mode=read`, which is the rule `PRICING.md` §6 already sets: creation, not access. It lives at `/pro/custom-tongue-twisters`, with the editor under it; `/genres/yours` lists prompts only. The page is noindex, because signed in it lists somebody's own writing: `/pro` is the surface that sells and is crawled. A prompt is a line and a passage is a paragraph, so the paste splits on blank lines, carries no style tail, takes a difficulty rather than a coined style, holds fifty rather than two hundred, and takes no pictures. The mode is asked once when the genre is made and never again.
- **Which bank you draw from is a setting**, in the Passages row beside difficulty, and your own warm-ups sit there next to the built-in one. It narrows what a spin lands on rather than changing what the round is, so it does not earn a URL. The row is absent when there is nothing to choose.
- **The way in routes by what you already are**: signed out to `/login?next=/pro`, signed in without Pro to `/pro`, Pro to the editor. Nobody meets a checkout they cannot complete or a paywall they have already paid.

### The bank

Over 1500 topics across the ten genres, one JSON file per genre in the backend, loaded by an idempotent seeder that owns every column it touches on built-in rows, upserts by text, never deletes a topic (`is_active` off instead) and never reads past `owner IS NULL`. It is a management command run once by hand, never a boot step: nothing seeds itself, and compose only migrates. Text and slug are unique per genre, not globally, because two owners may both write the same sentence.
...
- **Days are the visitor's own local days.** Every run stores the UTC offset of the clock that made it and is dated by that clock, so a round finished at 11pm on Tuesday counts as Tuesday for the person who spoke, whatever the server thinks and wherever they travel after. Today is the clock of the request asking: the round's own offset on the write, the timezone cookie on a page load.
- **The rule is the plan's.** Free tracks five days: the streak never reads above five and a missed day ends it. Pro tracks as many days as the plan lasts (thirty for a month or the pass, 365 for a year, 365 for lifetime) and gets the freeze rule. When Pro lapses the free rule takes over. Owner's call, 2026-09-07. [`PRICING.md`](PRICING.md) §2, §6.
- **Asked once per request and reused.** One summary (streak, longest, totals, would-be, frozen days) is computed from one read of the days, and the header pill, the done screen and `/streak` all read it, so no page can disagree with another. The pill on every page reads `GET /api/v1/runs/summary`; the done screen reads the answer to its own POST.
...
- **Freezes are a counting rule, not an inventory.** A Pro streak survives two missed days in any thirty, counted back from the gap rather than per calendar month, so the same absence is forgiven or not wherever in the month it falls. No table, no balance, no SKU, nothing to build a "two left" control for. All or nothing: a gap that cannot be paid for in full spends nothing. Buying Pro repairs the gaps already behind you, because the answer was always derivable and only the rule changed. [`PRICING.md`](PRICING.md) §6.
...
- **Sharing** is one nullable token on the account: 16 random bytes, minted from the streak page and never rotated while sharing is on. `/s/<token>` shows the name if given (never the email), the three tiles, eight weeks (its own constant, not the free window: a shared page is not a tier, and a lapsed Pro's link keeps working) and the bank topics recently practised, each linking `/?topic=`; no account needed to view, `noindex`. Turning it off lives in the account's additional settings, not on the streak page, which is the page you come to in order to show somebody (owner's call on card 18, changing v0, which had no off switch at all); off clears the token, so the link already sent stops working at once and sharing again mints a new one, because an off switch that parked the token would let everybody holding the old link back in the moment sharing went on again. The admin can clear a token by hand too.
...
- **`/account`** holds the subscription, purchases and the colour (six accents, Pro only) as sections; `/account/additional-settings` holds the name, the email, the password, sharing, signed-in browsers and closing the account. Anything new is a section on a page, not a menu entry. Every section is a card that saves itself and says so beside the button that did it; the two pages sit under one tab bar naming both.
...
- **PostHog** runs with its own current defaults on (autocapture, pageviews, session replay as the project configures them) when the frontend's `.env` says `DEBUG_ENV=production` and holds a token; either alone sends nothing, and the switch is a word in env rather than the build mode so a production build run on a laptop is not traffic. The distinct id is the device, because it is what runs join on and the only id somebody has before they sign up; signed in, the address and the name ride on identify as person properties, so a recording is found by the person who wrote in. The one thing the site adds to the defaults is a replay mask on the class the prep notes carry into the speak phase, since chips are not inputs and the default input masking cannot reach them; a mask naming a class nothing carries fails silently, so a test pins the markup and the privacy page states the behaviour. Events fired before the library lands are queued, because a visitor can spin inside two seconds.
- **Crisp** draws the support bubble on every page except the home page, which is the tool, and hides with the chrome while filming. Signed in, the address and the name ride on the session so a conversation can be put to a person. The footer leaves room for the bubble on exactly the pages that show it.
...
- **Header**: the die and the wordmark, "yapholic" in ink, written out on a phone too; the "Features" link; the streak pill; the account menu. The band runs full width and carries the hairline; the row inside stops at 80rem.
